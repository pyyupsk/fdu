---
title: Format Tokens
description: Complete reference guide for all available date and time format tokens in @pyyupsk/fdu. Master YYYY, MM, DD, HH, mm, ss tokens for year, month, day, hour, minute, second formatting with examples, locale-aware patterns, escape sequences, and common formatting patterns like ISO 8601 and RFC 2822.
---

The `.format()` method uses tokens to format dates. Here's the complete list of supported tokens.

<Callout type="info">
  All code examples in this guide assume you've imported `fdu`: ```typescript
...
See the [Localization](/docs/guides/i18n) guide for more information about using locales.
