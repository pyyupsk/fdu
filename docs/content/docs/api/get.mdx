...
<Callout type="info">
  The `day()` method can also be used as a setter. See [day()
  setter](/docs/api/manipulation/day) for manipulation examples.
</Callout>
...
<Callout type="info">
  Unlike `day()`, `weekday()` respects the locale's week start setting. The
  `weekday()` method can also be used as a setter. See [weekday()
  setter](/docs/api/manipulation/weekday) for manipulation examples.
</Callout>
...
monday.weekday(); // 1 (Monday is second day of week)
```
...
dateCustom.weekday(); // 6 (last day of week in Monday-start locale)
```
